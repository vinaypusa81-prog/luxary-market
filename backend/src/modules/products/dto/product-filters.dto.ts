...
export class ProductFiltersDto {
  ...
  @ApiPropertyOptional() @IsOptional() @Type(() => Number) @IsNumber() @Min(0) @Max(100) discount?: number;
  ...
  @ApiPropertyOptional() @IsOptional() @Transform(({ value }) => value?.split(',')) @IsArray() colors?: string[];
  @ApiPropertyOptional() @IsOptional() @Transform(({ value }) => value?.split(',')) @IsArray() sizes?: string[];
  @ApiPropertyOptional({ enum: ['price', 'rating', 'newest', 'popularity', 'discount', 'relevance'] })
  @IsOptional() @IsString() sortBy?: string;
  @ApiPropertyOptional({ enum: ['asc', 'desc'] }) @IsOptional() @IsString() sortOrder?: string;
